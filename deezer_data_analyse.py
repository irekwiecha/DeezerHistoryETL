...
most_track_artist = df.artist_name.where(df.song_name == most_track).dropna().head(1).item()
most_artist = df.artist_name.value_counts().nlargest(1).index[0]
most_day = df.date.value_counts().nlargest(1).index[0].date()
summary_dict = {
...
genre = df['genre'].value_counts().where(df['genre'].value_counts() > 20).dropna()
labels = genre.index

artist_by_genre = {}
group_genre = df.groupby('genre')
for label in labels:
    artist_by_genre[label] = group_genre.get_group(label).artist_name.value_counts().index[0]


summary_by_genre = pd.DataFrame.from_dict(artist_by_genre, orient='index', columns=['the most played artist by genre'])
